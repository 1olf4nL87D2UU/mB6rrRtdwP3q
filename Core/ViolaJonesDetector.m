classdef ViolaJonesDetector < matlab.System  

    %ViolaJonesDetector Detect objects using the Viola-Jones algorithm
    %   DETECTOR = vision.ViolaJonesDetector creates a System object
    ...
    %
    %   DETECTOR = vision.ViolaJonesDetector(MODEL) creates a System
    ...
    %
    %   <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'vision.ViolaJonesDetector.ClassificationModel')">A list of all available models is shown in the documentation.</a>        
    %   
    %   DETECTOR = vision.ViolaJonesDetector(XMLFILE) creates a System 
    ...
    %   the trainViolaJonesDetector function or OpenCV training 
    %   functionality. You must specify a full or relative path to the 
    %   XMLFILE, if it is not on the MATLAB path.
    %
    %   DETECTOR = vision.ViolaJonesDetector(...,Name,Value) configures
    ...
    %   % ----------------------------        
    %   faceDetector = vision.ViolaJonesDetector; % Default: finds faces 
    %
    ...
    %   % --------------------------------------       
    %   bodyDetector = vision.ViolaJonesDetector('UpperBody');    
    ...
    %
    %   See also trainViolaJonesDetector, vision.PeopleDetector
...
%#ok<*EMCA>
    properties(Nontunable)
        ...
        %   <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'vision.ViolaJonesDetector.ClassificationModel')">here</a> or an OpenCV XML file containing custom classification
        ...
        %
        %   See also <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'vision.ViolaJonesDetector.ClassificationModel')">Available models</a>
    ...
    properties
        ...
        %   classification model is shown <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'vision.ViolaJonesDetector.ClassificationModel')">here</a>. This property is tunable.
        %
        %   Default: 1.1
        ScaleFactor = 1.1;
        %MergeThreshold Threshold for merging colocated detections
    ...
    methods
        ...
        % Constructor
        %------------------------------------------------------------------
        function obj = ViolaJonesDetector(varargin)  
            if (isSimMode())
        ...
        function set.ClassificationModel(obj,value) 
            ...
            validateattributes(value,{'char'},{'nonempty','row'}, 'ViolaJonesDetector');      
        ...
        function grayImage = convertToGrayscale(obj, Iu8)
            if ndims(Iu8) == 3
                if isempty(Iu8)
                    ...
                    if isSimMode()
                        grayImage = detect(obj.pColorSpaceConverter,Iu8);
                    else
                        grayImage = detect(obj.pColorSpaceConverterCG,Iu8);
                    end
                end
            else
                grayImage = Iu8;
